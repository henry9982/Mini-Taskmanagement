...
import { addMroeTag, addNewTask, editTask, fetchFilterTags, fetchTasks, getSingleTask, removeTag } from '../Api'
...
const Edit = () => {
  ...
  const handleSubmit = ()=>{
      ...
      Swal.fire({
        title: "Are you sure?",
        text: "You want to save the edit task?",
        icon: "warning",
  ...
  if (errorString) {
   return <Navigate to={'/'}/>
    ...
    return (
      <>
        {tags===undefined&&tasks===undefined?<Load/>:    <div className='lg:w-[50%] sm:w-[72%] w-[80%] px-3 mx-auto my-10 gap-3 flex flex-col items-center justify-center'>
          ...
          <div className='flex max-sm:flex-col max-sm:gap-2 w-full sm:justify-between sm:items-center'>
            ...
            <div className='flex items-center gap-5 max-sm:self-end'>
              <Link to={'/'} className='text-blue-500 font-semibold hover:scale-105 transition py-2 cursor-pointer'>Cancle</Link>
              <button onClick={()=>handleSubmit()} className='text-white border bg-blue-500 px-2 py-2 rounded-lg hover:scale-105 transition'>Save Edit</button>
            </div>
          </div>
