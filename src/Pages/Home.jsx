...
const Home = () => {
  ...
  return (
    <div className='flex overflow-hidden lg:w-[60%] md:w-[75%] w-full mx-auto  py-10 px-5 gap-3  flex-col'>
        ...
        <div className='flex w-full overflow-hidden flex-nowrap whitespace-nowrap overflow-x-auto border scroll-bar py-3 cursor-pointer px-3 rounded items-center gap-2'>
            <div>Tags:</div>
              <div className='flex flex-nowrap gap-2'>
                {tags.map(tag=><NavLink key={tag.id} to={tag.filterBy===''?'/':tag.filterBy} className="border text-sm rounded px-1 py-1 hover:scale-110 transition hover:bg-black hover:text-white ">{tag.tag}</NavLink>)}
              </div>
          </div>
        <Outlet/>
    </div>
